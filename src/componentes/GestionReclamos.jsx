...
import { 
  ...
  Trash2, Archive, CheckCircle, AlertTriangle, Calendar
} from 'lucide-react';
import { format } from 'date-fns';
import toast from 'react-hot-toast';

// ==========================================================
// 1. COMPONENTE MODAL (ESTILO COHERENTE CON USUARIOS)
// ==========================================================
const ReclamoDetailModal = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-[#3b0f52]/60 backdrop-blur-md flex justify-center items-center z-50 p-4 animate-in fade-in duration-300" onClick={onClose}>
      <div className="bg-white rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.3)] max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        
        <div className="bg-[#7e1d91] p-8 text-white flex justify-between items-center">
          <div>
            <span className="text-[10px] font-black uppercase tracking-[0.4em] opacity-70">Atención al Cliente</span>
            <h2 className="text-2xl font-black italic mt-1 uppercase">Detalle de Incidencia</h2>
          </div>
        ...
        <div className="overflow-y-auto p-8 lg:p-12 space-y-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="bg-[#fcfaff] p-6 rounded-[2.5rem] border border-[#ecd8ff] shadow-inner">
                   <p className="text-[9px] font-black text-[#7e1d91]/40 uppercase tracking-widest mb-2">Comentario del Cliente</p>
                   <p className="text-sm text-[#3b0f52] font-bold leading-relaxed italic">"{reclamo.comentario}"</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              <div>
                <h3 className="text-[10px] font-black text-[#7e1d91] uppercase tracking-[0.3em] mb-4">Actualizar Estado</h3>
                <div className="grid grid-cols-3 gap-2">
                  {estados.map((est) => (
                    ...
                    >
                      {est.icon}
                      <span className="text-[8px] font-black uppercase">{est.label}</span>
                    </button>
                  ))}
                </div>
              </div>
...
// ==========================================================
// 2. COMPONENTE PRINCIPAL (PAGINACIÓN DE 5 REGISTROS)
// ==========================================================
export default function GestionReclamos() {
  ...
  const StatusBadge = ({ status }) => (
    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase italic border-2 ${
      status === 'respondido' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
      status === 'archivado' ? 'bg-gray-50 text-gray-500 border-gray-100' :
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-32 gap-4">
      ...
      <p className="text-[#3b0f52] font-black uppercase tracking-[0.3em] text-[10px] italic">Sincronizando Libro...</p>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-8 p-4">
      ...
      <div className="bg-white p-5 rounded-[2.5rem] border border-[#ecd8ff]/60 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="relative w-full md:max-w-md">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-[#7e1d91]/30" size={18} />
          <input 
            type="text" 
      ...
      <div className="bg-white rounded-[3rem] border border-[#ecd8ff] shadow-xl overflow-hidden min-h-[400px]">
        ...
        <div className="md:hidden p-5 space-y-4">
          {itemsActuales.map((r) => (
            <div key={r.id} className="bg-[#fcfaff] rounded-[2.5rem] border border-[#ecd8ff]/40 p-6 space-y-4 relative">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="font-black text-[#3b0f52] uppercase italic text-sm truncate">{r.nombres} {r.apellidos}</p>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                    {r.fechaFormateada ? format(r.fechaFormateada, "dd/MM/yyyy") : "S/F"}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between pt-3 border-t border-[#ecd8ff]/30">
                ...
                <button 
                  onClick={() => { setSelectedReclamo(r); setRespuesta(r.respuestaAdmin || ''); setEstado(r.estado || 'pendiente'); }}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#fcfaff] border-b border-[#f0e4ff]">
                <th className="px-10 py-7 text-[10px] font-black text-[#7e1d91] uppercase tracking-[0.2em] text-left">Cliente</th>
                <th className="px-10 py-7 text-[10px] font-black text-[#7e1d91] uppercase tracking-[0.2em] text-center">Estado</th>
                <th className="px-10 py-7 text-[10px] font-black text-[#7e1d91] uppercase tracking-[0.2em] text-left">Fecha</th>
                <th className="px-10 py-7 text-[10px] font-black text-[#7e1d91] uppercase tracking-[0.2em] text-right">Acción</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#f7f1ff]">
              {itemsActuales.map((r) => (
                <tr key={r.id} className="group hover:bg-[#fcfaff]/50 transition-all">
                  <td className="px-10 py-5">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-[#3b0f52] text-white rounded-[15px] flex items-center justify-center font-black text-xs italic">
                        {r.nombres[0]}{r.apellidos[0]}
                      </div>
                      <span className="font-black text-[#3b0f52] text-sm uppercase italic">{r.nombres}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-10 py-5 text-[11px] font-black text-gray-400 uppercase italic">
                    {r.fechaFormateada ? format(r.fechaFormateada, "dd MMM, yyyy") : "---"}
                  </td>
