...
export default function Responder() {
  ...
  const getStatusBadge = (estado) => {
    ...
    if (s === 'aceptado' || s === 'respondido') 
      return <span className={`${baseClass} bg-emerald-50 text-emerald-500 border border-emerald-100`}><CheckCircle2 size={12}/> ¡Aceptado!</span>;
    
    if (s === 'denegado') 
      return <span className={`${baseClass} bg-red-50 text-red-400 border border-red-100`}><XCircle size={12}/> Finalizado</span>;

    if (s === 'procesado') 
      return <span className={`${baseClass} bg-blue-50 text-blue-500 border border-blue-100`}><Loader2 size={12} className="animate-spin"/> En Revisión</span>;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-6">
        ...
        <p className="text-[#3b0f52] font-black uppercase text-[10px] tracking-[0.3em]">Cargando tus respuestas...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-10">
      ...
      <div className="relative bg-white rounded-[35px] md:rounded-[50px] p-6 md:p-10 mb-6 md:mb-10 shadow-xl border border-purple-50 overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-4 md:gap-8 text-center md:text-left">
            ...
            <div>
                ...
                <p className="text-[#6b4a88] font-semibold opacity-50 text-[10px] md:text-sm tracking-wide uppercase">Estado de tus trámites y empleos</p>
            </div>
        </div>
      </div>
      ...
      <div className="flex bg-white p-1.5 md:p-2.5 rounded-[25px] md:rounded-[35px] shadow-sm border border-gray-100 mb-6 md:mb-10">
        ...
        >
          <Briefcase size={16} className="md:w-5 md:h-5"/> Postulaciones
        </button>
      </div>
      ...
      <div className="space-y-4 md:space-y-6 min-h-[300px]">
        {visibleItems.length === 0 ? (
          <div className="text-center py-16 md:py-24 bg-white rounded-[35px] border-2 border-dashed border-gray-100">
              <p className="text-gray-300 font-black uppercase italic text-[10px] tracking-widest">No hay registros aún</p>
          </div>
        ) : (
              ...
              <div className="flex flex-col gap-4 md:gap-6">
                ...
                <div className="flex items-center gap-4 md:gap-6">
                  ...
                  <h4 className={`font-black text-[#3b0f52] italic uppercase tracking-tighter leading-tight break-words flex-1 ${activeTab === 'empleo' ? 'text-2xl md:text-4xl' : 'text-xl md:text-3xl'}`}>
                    {activeTab === 'reclamos' 
                      ? (item.tipo || 'Consulta General') 
                      : (vacantes[item.puestoId] || item.puestoVisual || 'Posición Solicitada')
                    }
                  </h4>
                </div>

                {activeTab === 'reclamos' && item.comentario && (
                ...
                {item.respuestaAdmin && (
                  <div className="bg-purple-50 p-4 md:p-6 rounded-[20px] md:rounded-[30px] border border-purple-100 flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-[9px] font-black text-purple-300 uppercase italic">Respuesta del equipo:</p>
      ...
      {totalPages > 1 && (
        <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 mt-10 md:mt-16 pb-8">
          <button
            ...
            className="p-3 md:p-4 rounded-full bg-white text-[#7e1d91] disabled:opacity-20 shadow-md transition-all hover:scale-110"
          >
